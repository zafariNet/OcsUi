(function ($) {
  ...
  $.fn.simpleXML = function (options) {
    ...
    if (xml == undefined) xml = $.parseXML(settings.xmlString);
    ...
    return this.each(function () {
      ...
      $(wrapperNode)
        .find(".simpleXML-expanderHeader")
        .click(function () {
          var expanderHeader = $(this).closest(".simpleXML-expanderHeader");

          var expander = expanderHeader.find(".simpleXML-expander");

          var content = expanderHeader
            .parent()
            .find(".simpleXML-content")
            .first();
          var collapsedText = expanderHeader
            .parent()
            .children(".simpleXML-collapsedText")
            .first();
          var closeExpander = expanderHeader
            .parent()
          ...
          if (expander.hasClass("simpleXML-expander-expanded")) {
            ...
            expander
              .removeClass("simpleXML-expander-expanded")
              .addClass("simpleXML-expander-collapsed");
            ...
            // Time to expand..
            expander
              .addClass("simpleXML-expander-expanded")
              .removeClass("simpleXML-expander-collapsed");
  ...
  function showNode(parent, xml, settings) {
      ...
      case 1: // Simple element
        {
          ...
          var expanderHeader = expandingNode
            ? makeSpan("", "simpleXML-expanderHeader")
            : parent;
          ...
          expanderHeader.appendChild(
            makeSpan(xml.nodeName, "simpleXML-tagValue")
          );

          if (expandingNode) parent.appendChild(expanderHeader);
          ...
          for (var attrIdx = 0; attrIdx < attributes.length; attrIdx++) {
            ...
            expanderHeader.appendChild(
              makeSpan(attributes[attrIdx].name, "simpleXML-attrName")
            );
            expanderHeader.appendChild(makeSpan('="'));
            expanderHeader.appendChild(
              makeSpan(attributes[attrIdx].value, "simpleXML-attrValue")
            );
            expanderHeader.appendChild(makeSpan('"'));
          }
          ...
          if (hasChildNodes) {
            ...
            if (expandingNode) {
              ...
              var collapsedTextSpan = makeSpan(
                settings.collapsedText,
      ...
      case 8: // comment
        {
          parent.appendChild(makeSpan("", "simpleXML-expander"));
          parent.appendChild(
            makeSpan("<!--" + xml.nodeValue + "-->", "simpleXML-comment")
          );
        }
        break;
    ...
    function makeSpan(innerText, classes) {
      ...
      if (classes != undefined) span.setAttribute("class", classes);
